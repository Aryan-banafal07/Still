  ...
  const ProductCard = ({ product, index }) => {
    ...
    useEffect(() => {
      ...
      if (isHovered) {
        ...
        const videoTimer = setTimeout(() => setShowVideo(true), product.images.length * 800);
    ...
    return (
      <div
        className={`group transition-all duration-1000 bg-white hover:shadow-2xl cursor-pointer ${
      ...
      >
        <div className="relative w-full h-80 lg:h-96 overflow-hidden">
          ...
          <div className="absolute top-4 left-4">
            <span className="bg-white/90 text-gray-700 text-xs px-3 py-1 uppercase tracking-wide font-light backdrop-blur-sm">
              {product.category}
            </span>
          </div>
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <button className="bg-white/95 px-6 py-2 text-sm text-gray-900 font-light hover:bg-white transition-colors duration-300 shadow-lg backdrop-blur-sm transform translate-y-4 group-hover:translate-y-0">
              Quick Shop
            </button>
          </div>
            ...
            {product.images.map((_, idx) => (
              <div
                ...
                className={`w-1.5 h-1.5 rounded-full ${
                  currentImageIndex === idx && !showVideo ? 'bg-white' : 'bg-white/40'
                }`}
              />
            ))}
            <div
              className={`w-1.5 h-1.5 rounded-full ${showVideo ? 'bg-white' : 'bg-white/40'}`}
            />
          </div>
        </div>

        <div className="p-6 space-y-3">
          <h3 className="text-lg text-gray-900 font-light group-hover:text-gray-600 transition-colors duration-300">
            {product.name}
          </h3>
          <p className="text-lg text-gray-700 font-light">{product.price}</p>
          <div className="pt-2 w-full h-px bg-gray-100 group-hover:bg-gray-200 transition-colors duration-300" />
        </div>
      </div>
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Carefully curated pieces that embody our philosophy of timeless design and
